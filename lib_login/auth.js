...
router.get("/register", function (request, response) {
  ...
  var html = template.HTML(
    ...
    <p><input class="login" type="text" name="userbirth" placeholder="생년월일"></p>
    <p><input class="login" type="text" name="usergender" placeholder="성별"></p>    
...
router.post("/register_process", function (request, response) {
  ...
  var userbirth = request.body.userbirth;
  var usergender = request.body.usergender;

  if (username && password && password2 && userbirth && usergender) {
    db.query(
      ...
      function (error, results, fields) {
        ...
        if (results.length <= 0 && password == password2) {
          ...
          db.query(
            "INSERT INTO user (username, password,userbirth,usergender) VALUES(?,?,?,?)",
            [username, password, userbirth, usergender],
            function (error, data) {
